...
describe("Cancel itOperation", () => {
  ...
  it("Sends a valid Request", (done) => {
    ...
    request(app)
      .patch("/api/itOperation/cancel/" + itOperationId.toString()+"?sendMail=false")
      .send({"notes":"abcd66"})
  ...
  it("Sends an invalid request - Wrong ID", async () => {
    it("Sends a valid Request", async () => {
      ...
      const res = await request(app)
        .put("/api/itOperation/cancel/" + itOperationWrongId +"?sendMail=false")
        .send({"notes":"abcd66"})
        .set("content-type", "application/json");
  ...
  it("Sends an invalid request - missing query", async () => {
    it("Sends a valid Request", async () => {
      const itOperationWrongId = "60ef4e3f7221301cc8eaafbe";
      const res = await request(app)
        .put("/api/itOperation/cancel/" + itOperationWrongId)
        .set("content-type", "application/json");
      expect(res.status).to.equal(400);
    });
  });
