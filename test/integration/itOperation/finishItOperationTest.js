...
describe("Finish itOperation", () => {
  ...
  it("Sends a valid Request", (done) => {
    ...
    request(app)
      .patch("/api/itOperation/finish/" + itOperationId.toString()+"?sendMail=false")
  ...
  it("Sends an invalid request - Wrong ID", async () => {
    it("Sends a valid Request", async () => {
      ...
      const res = await request(app)
        .put("/api/itOperation/finish/" + itOperationWrongId +"?sendMail=false")
        .set("content-type", "application/json");
  ...
  it("Sends an invalid request - missing query", async () => {
    it("Sends a valid Request", async () => {
      const itOperationWrongId = "60ef4e3f7221301cc8eaafbe";
      const res = await request(app)
        .put("/api/itOperation/finish/" + itOperationWrongId)
        .set("content-type", "application/json");
      expect(res.status).to.equal(400);
    });
  });
