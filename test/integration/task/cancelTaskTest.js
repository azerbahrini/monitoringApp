...
let task2;
let task3;
describe("Cancel Task controller Test", () => {
  ...
  it("Sends a valid Request", (done) => {
    ...
    request(app)
      .put("/api/task/cancelTask/" + taskId.toString())
      ...
      .then((res) => {
        ...
        expect(res.body.data.state).to.be.eq("Canceled");
        expect(res.body).to.have.property("message");
        expect(res.body.message).to.be.a("string");
        expect(res.body.message).to.be.eq("Successfully Canceled the task");
        done();
      })
      .catch((err) => done(err));
  });
  ...
  it("returns a 404 error", (done) => {
    ...
    request(app)
      .put("/api/task/cancelTask/" + taskId)
      .set("content-type", "application/json")
      .then((res) => {
  ...
  it("returns a 400 error", (done) => {
    ...
    request(app)
      ...
      .then((res) => {
        ...
        expect(res.body).to.have.property("message");
        expect(res.body.message).to.be.a("string");
        expect(res.body.message).to.be.eq("The Task is already Canceled");
        done();
      })
      .catch((err) => done(err));
  });
