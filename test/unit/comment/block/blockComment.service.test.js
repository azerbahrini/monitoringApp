...
const blockComment = require('../../../../services/comment/blockComment.service');
const Comment = require('../../../../models/Comment');

chai.use(sinonChai);
describe('block Comment service TEST', () => {
  let updateStub;
  let sendInfoStub;
  const sandbox = sinon.createSandbox();
  beforeEach(() => {
    updateStub = sandbox.stub(Comment, 'findOneAndUpdate');
    sendInfoStub = sandbox.stub(producer, 'sendInfo');
  });
  afterEach(() => {
    updateStub.restore();
    sendInfoStub.restore();
  });
  it('Expect to return a success', async () => {
    ...
    sendInfoStub.returns({
      message: 'Comment blocked',
      data: fixture.updatedComment
    });
    const res = await blockComment(fixture.document._id);
  ...
  it('Expect to return an Error - 404 not found', async () => {
    ...
    const res = await blockComment(fixture.invalidID);
    expect(updateStub).to.have.been.calledOnce;
  ...
  it('Expect to return an error', async () => {
    ...
    const res = await blockComment(fixture.documentWithoutID);
    expect(updateStub).to.have.been.calledOnce;
    expect(res).to.be.a('object');
    expect(res).to.have.property('err');
    expect(res).to.have.property('status');
