...
describe('controller blockComment Test ', () => {
  let stubBlock;
  let stubGetRoles;
  let stubGet;
  const token =
  ...
  beforeEach(() => {
    stubBlock = sinon.stub(commentService, 'blockComment');
  ...
  it('expect to return success ', async () => {
    stubBlock.returns(fixture.blockedResult);
    ...
    await commentController.blockComment(req, res);
    expect(stubBlock).to.be.calledWith(fixture.document._id);
