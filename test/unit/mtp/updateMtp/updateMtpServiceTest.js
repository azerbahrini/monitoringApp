...
const updateMtpService = require('../../../../services/mtp/updateMtpService');

const MTP = require('../../../../models/MTP');

chai.use(sinonChai);

describe('testing Update MTP service', () => {
  const sandbox = sinon.createSandbox();
  let findStub;
  beforeEach(() => {
    findStub = sandbox.stub(MTP, 'findOneAndUpdate');
  });
  afterEach(() => {
    findStub.restore();
  });
  ...
  it('expect to return an success object', async () => {
    ...
    const res = await updateMtpService('6212a1468a338608587cd325', {isActive: true});
  ...
  it('Expect to throw an exception', async () => {
    ...
    const res = await updateMtpService();
    expect(findStub).to.have.been.calledOnce;
    expect(res).to.be.a('object');
