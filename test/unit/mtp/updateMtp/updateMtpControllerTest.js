...
describe('controller UPDATE MTP test ', () => {
  let stubUpdate;
  beforeEach(() => {
    stubUpdate = sinon.stub(mtpService, 'updateMtpService');
  });
  afterEach(() => {
    stubUpdate.restore();
  });
  it('expect to return an Updated Mtp ', async () => {
    ...
    await mtpController.updateMTP(req, res);
    expect(stubUpdate).to.be.calledWith({ _id: '6212a1468a338608587cd325' }, { estimatedStart: '20:20' });
  ...
  it('expect to return an Updated Mtp route+post+ip', async () => {
    stubUpdate.returns({
        data: {
            'isActive': true,
            '_id': '6212a1468a338608587cd325',
