...
const getByIdSlaContractService = require("../../../../services/slaContract/getSlaContractById.service");
const SlaContract = require("../../../../models/SlaContract");

chai.use(sinonChai);

describe("GET BY ID SlaContract service", () => {
  const sandbox = sinon.createSandbox();
  let findOneStub;
  beforeEach("", () => {
    findOneStub = sandbox.stub(SlaContract, "findOne");
  });
  it("Expect to return an success object", async () => {
    ...
    expect(fixture.slaContractDataTest._id).to.be.a("string");
    const res = await getByIdSlaContractService(
  ...
  it("Expect to return an fail object - wrong id", async () => {
    ...
    const res = await getByIdSlaContractService(fixture.wrongID);
    expect(findOneStub).to.have.been.calledOnce;
    expect(findOneStub).to.be.calledWith({
      _id: fixture.wrongID,
      isActive: true,
